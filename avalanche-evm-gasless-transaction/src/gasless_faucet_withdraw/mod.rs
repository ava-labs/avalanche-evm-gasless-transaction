...
pub const NAME: &str = "gasless-faucet-withdraw";

pub fn command() -> Command {
    Command::new(NAME)
        .about("Withdraws/deposits the native token")
        .arg(
            Arg::new("LOG_LEVEL")
                .long("log-level")
                .short('l')
        ...
        .arg(
            Arg::new("WITHDRAW_AMOUNT_IN_HEX")
                .long("withdraw-amount-in-hex")
                .help("Withdraw amount in hex")
                .required(false)
                .num_args(1)
                .default_value("0x123456789"),
        )
        .arg(
            Arg::new("SKIP_PROMPT")
                .long("skip-prompt")
...
pub async fn execute(
    ...
    if !skip_prompt {
        let options = &[
            format!("No, I am not ready to withdraw with the recipient contract {recipient_contract_address}."),
            format!("Yes, let's withdraw with the recipient contract {recipient_contract_address}."),
        ];
        let selected = Select::with_theme(&ColorfulTheme::default())
            .with_prompt("Select your 'gasless-faucet-withdraw' option")
            .items(&options[..])
            .default(0)
            .interact()
            .unwrap();
    ...
    let arg_tokens = vec![Token::Uint(
        U256::from_str_radix(withdraw_amount_in_hex, 16).unwrap(),
    )];
